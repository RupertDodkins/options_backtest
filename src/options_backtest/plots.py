...
def plot_profits(df_list, offsets, kwargs={}):
    ...
    colors = px.colors.sample_colorscale("viridis", [n / (n_colors) for n in range(n_colors)])

    fig = go.Figure(data=[go.Scatter(x=df_list[0]['date'],
                                     y=df_list[0]['running_profit_$'],
                                     name=str(offsets[0]),
                                     line_color=colors[0])])

    for i in range(1, len(df_list)):
        fig.add_scatter(x=df_list[i]['date'],
                        y=df_list[i]['running_profit_$'],
                        name=str(offsets[i]),
                        line_color=colors[i])
...
def overlap_plots(df_list, offsets, kwargs={}):
    ...
    fig = go.Figure(data=[go.Scatter(y=df_list[0]['running_profit_$'],
                                     name=str(offsets[0]),
                                     line_color=colors[0])])
